  ...
  return (
    <div id="portfolio-container" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 transition-all">
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between mb-12 gap-6">
        <div>
          <span className="text-xs font-mono tracking-widest text-brand-600 dark:text-brand-400 uppercase">Selected Work</span>
          <h1 className="text-4xl sm:text-5xl font-display font-semibold tracking-tight text-slate-900 dark:text-slate-105 mt-2">
            Signature Portfolios
          </h1>
          <p className="text-base text-slate-500 dark:text-slate-400 mt-3 max-w-xl font-light">
            A minimalist showcase of modern digital products, interactive workflows, and brand experience architectural blueprints.
          </p>
        </div>

        {/* Dynamic counter */}
        <div className="hidden sm:flex items-center gap-3 bg-white dark:bg-slate-900 px-4 py-2 rounded-xl border border-slate-100 dark:border-slate-800 shadow-xs">
          <FolderGit2 className="w-4 h-4 text-brand-500 dark:text-brand-400" />
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl p-4 mb-8 flex flex-col md:flex-row gap-4 items-start md:items-center justify-between shadow-xs">
        <div className="flex flex-wrap gap-2 items-center">
          <div className="flex items-center gap-2 text-xs font-mono text-slate-400 dark:text-slate-550 mr-2 uppercase">
          ...
          {statuses.map(status => (
            <button
              ...
              className={`px-3 py-1 text-xs font-medium rounded-lg capitalize transition-all cursor-pointer ${
                activeStatus === status
                  ? 'bg-slate-700 dark:bg-slate-200 text-white dark:text-slate-900 shadow-xs'
                  : 'bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700'
              }`}
            >
              {status}
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {[1, 2, 3].map(i => (
            <div key={i} className="border border-slate-100 dark:border-slate-800 rounded-2xl overflow-hidden bg-white dark:bg-slate-900 p-5 space-y-4 animate-pulse">
              <div className="w-full h-44 bg-slate-100 dark:bg-slate-800 rounded-xl"></div>
              <div className="flex justify-between items-center">
                <div className="w-1/4 h-3 bg-slate-100 dark:bg-slate-800 rounded"></div>
                <div className="w-1/5 h-3 bg-slate-100 dark:bg-slate-800 rounded"></div>
              </div>
              <div className="w-3/4 h-5 bg-slate-200 dark:bg-slate-700 rounded mt-2"></div>
              <div className="space-y-2 pt-2">
                <div className="w-full h-3 bg-slate-50 dark:bg-slate-800 rounded"></div>
                <div className="w-5/6 h-3 bg-slate-50 dark:bg-slate-800 rounded"></div>
              </div>
              <div className="pt-4 border-t border-slate-50 dark:border-slate-800 flex justify-between items-center">
                <div className="w-1/3 h-3 bg-slate-100 dark:bg-slate-800 rounded"></div>
                <div className="w-1/4 h-3 bg-slate-205 dark:bg-slate-700 rounded"></div>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="text-center py-20 bg-white dark:bg-slate-900 border border-dashed border-slate-200 dark:border-slate-800 rounded-3xl p-8">
          <FolderGit2 className="w-12 h-12 text-slate-300 dark:text-slate-600 mx-auto mb-4" />
          <h3 className="text-lg font-medium text-slate-900 dark:text-slate-200">No project assets match</h3>
